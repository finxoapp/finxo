...
# Fill in AUTH_SECRET, AUTH_GOOGLE_ID, AUTH_GOOGLE_SECRET, AUTH_ALLOWED_EMAILS
pnpm install
pnpm dev
```

The app runs at http://localhost:3000. Sign-in is restricted to the Google accounts
listed in `AUTH_ALLOWED_EMAILS`; all others are rejected on the callback.
...
| `AUTH_ALLOWED_EMAILS` | Comma-separated list of Google emails allowed to sign in (case-insensitive). |
